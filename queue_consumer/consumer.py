...
class Consumer:
    ...
    def supervise(self,
        ...
        if blocking:
            self._supervise(polling_time, stuck_time, stuck_limit)
        else:
            t = Thread(target=self._supervise,
                       args=(polling_time, stuck_time, stuck_limit),
                       daemon=True)
            t.start()
            return t

    def _supervise(self, polling_time, stuck_limit, stuck_time):
        while True:
            self._check_workers()
    ...
    def _check_handlers(self, time_limit):
        for handler, timestamp in list(self._handlers_pool.items()):
            ...
            self._stuck_handlers.append(handler)
